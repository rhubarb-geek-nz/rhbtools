...
do
	...
	for e in "#include <libusb-1.0/libusb.h>" "#include <libusb.h>"
	do
    	try_cc $d $SOCKLIBS <<EOF
#include <stdio.h>
$e
...
return libusb_init(&ctx) && argc &&  argv; }
EOF
    	RC="$?"
		if test "$RC" = "0"
		then
...
$FINDLIBS_REQUIRED_GLOBALS
int main(int argc,char **argv)
{  size_t s=0; static iconv_t i; $e char *p=argv[0];
	return (argc && argv && iconv(i,&p,&s,argv,&s)); }
EOF
    	RC="$?"
		if test "$RC" = "0"
		then
...
int main(int argc,char **argv) {
static id myId=0;
const char *p=object_getClassName(myId);
return argc && argv && p; }
EOF
		RC="$?"
		if test "$RC" = "0"
...
echo ICONVLIBS=\"$ICONVLIBS\"\;
echo MATHLIBS=\"$MATHLIBS\"\;
echo OBJCLIBS=\"$OBJCLIBS\"\;
echo XMLLIBS=\"$XMLLIBS\"\;
