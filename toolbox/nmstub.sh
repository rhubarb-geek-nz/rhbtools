...
#
# $Id: nmstub.sh 1 2014-02-06 21:56:41Z rhubarb-geek-nz $
#
...
nm "$LIBNAME" | while read ADDR SEG NAME
do
	if test "$NAME" != ""
	then
		if is_export "$NAME"
		then
			if test "$USCORE" != ""
			then
				NAME=`echo $NAME | sed s/$USCORE//`
			fi

			case "$SEG" in
			D )
				echo "char $NAME[sizeof(void *)]={1};"
				;;
			S )
				echo "char $NAME[sizeof(void *)];"
				;;
			T )
				STRLEN=`strlen "$NAME"`
				STRLEN=`echo $STRLEN`
