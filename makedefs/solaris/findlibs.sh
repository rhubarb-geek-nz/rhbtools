...
X11CONTENDERS="/usr/openwin /usr/X11R6 /opt/X11R6"
...
if test "$X11LIBPATH" != ""
then
	X11APP=`arglist -Wl,-R, $X11LIBPATH`
	X11LIBPATH=`arglist -L $X11LIBPATH`

	MAKEDEFS_DEFS="$MAKEDEFS_DEFS X11APP X11LIBPATH"
...
MAKEDEFS_DEFS="$MAKEDEFS_DEFS HAVE_LIBUSB HAVE_LIBUSB1 HAVE_LIBPCSC HAVE_LIBUUID HAVE_LIBX11 HAVE_LIBXT HAVE_LIBXM HAVE_LIBEXPAT HAVE_LIBCURL HAVE_LIBCRYPTO HAVE_LIBSSL HAVE_LIBXML"
